...
# Define the Material model (using 'nickname' as the display field)
...
@app.route('/summary')
def summary():
    # Read the CSV and compute basic metrics
    file_path = current_project.get('file_path')
    try:
        df = pd.read_csv(file_path)
    except Exception as e:
        return f"<h2 style='color: red;'>Error reading the file: {e}</h2>"
    df['Area (sq in)'] = df['VGA Width in'] * df['VGA Height in']
    df['Perimeter (in)'] = 2 * (df['VGA Width in'] + df['VGA Height in'])
    df['Total Area (sq ft)'] = (df['Area (sq in)'] * df['Qty']) / 144
    df['Total Perimeter (ft)'] = (df['Perimeter (in)'] * df['Qty']) / 12
    df['Total Vertical (in)'] = (df['VGA Height in'] * df['Qty']).round(2)
    df['Total Horizontal (in)'] = (df['VGA Width in'] * df['Qty']).round(2)
    total_area = df['Total Area (sq ft)'].sum()
    total_perimeter = df['Total Perimeter (ft)'].sum()
    total_vertical = df['Total Vertical (in)'].sum()
    total_horizontal = df['Total Horizontal (in)'].sum()
    total_quantity = df['Qty'].sum()
    current_project['total_area_sqft'] = total_area
    current_project['total_perimeter_ft'] = total_perimeter
    current_project['total_vertical_in'] = total_vertical
    current_project['total_horizontal_in'] = total_horizontal
    current_project['total_quantity'] = total_quantity

    summary_table = f"""
    <table class="summary-table">
      <tr><th>Metric</th><th>Value</th></tr>
      <tr><td>Total Area (sq ft)</td><td>{total_area:.2f}</td></tr>
      <tr><td>Total Perimeter (ft)</td><td>{total_perimeter:.2f}</td></tr>
      <tr><td>Total Quantity</td><td>{total_quantity:.2f}</td></tr>
      <tr><td>Total Vertical (in)</td><td>{total_vertical:.2f}</td></tr>
      <tr><td>Total Horizontal (in)</td><td>{total_horizontal:.2f}</td></tr>
    </table>
    """
    table_html = df.copy().round(2).to_html(classes='data-table', index=False)
    return f"""
    <html>
      <head>
      ...
      <body>
         <div class="container">
            ...
            <button onclick="window.location.href='/materials'">Next: Materials &amp; Other Costs</button>
         </div>
      </body>
    </html>
    """
...
def materials():
    try:
        ...
        materials_glass = Material.query.filter_by(category=15).all()  # Glass
        materials_aluminum = Material.query.filter_by(category=1).all()  # Aluminum
        materials_glazing = Material.query.filter_by(category=2).all()  # Glazing Spline
        materials_gaskets = Material.query.filter_by(category=3).all()  # Gaskets
        materials_corner_keys = Material.query.filter_by(category=4).all()  # Corner Keys
        materials_dual_lock = Material.query.filter_by(category=5).all()  # Dual Lock
        materials_foam_baffle = Material.query.filter_by(category=6).all()  # Foam Baffle
        materials_glass_protection = Material.query.filter_by(category=7).all()  # Glass Protection
        materials_tape = Material.query.filter_by(category=10).all()  # Tape
    except Exception as e:
        return f"<h2 style='color: red;'>Error fetching materials: {e}</h2>"

    if request.method == 'POST':
        # Get selected IDs and options
        selected_glass = request.form.get('material_glass')
        aluminum_option = request.form.get('aluminum_option')  # "head_retainer" or "head_and_sill"
        ...
        tape_option = request.form.get('tape_option')  # "head_retainer" or "head_sill"

        # Retrieve material objects
        ...
        # Retrieve computed totals from current_project
        total_area = current_project.get('total_area_sqft', 0)
        total_perimeter = current_project.get('total_perimeter_ft', 0)
        total_vertical = current_project.get('total_vertical_in', 0)
        total_horizontal = current_project.get('total_horizontal_in', 0)
        ...
        cost_dual_lock = ((total_vertical / 12) * mat_dual_lock.cost) if mat_dual_lock else 0
        ...
        result_html = f"""
         <html>
           <head>
             <title>Material Cost Summary</title>
             <style>{common_css}</style>
           </head>
           <body>
             <div class="container">
               <h2>Material Cost Summary</h2>
               <table class="summary-table">
                 <tr>
                   <th>Category</th>
                 ...
                 <tr>
                   ...
                   <td>Total Vertical (in): {total_vertical:.2f} × Cost</td>
                 ...
                 <tr>
                   ...
                   <td>Total Vertical (ft): {(total_vertical / 12):.2f} × Cost</td>
                 ...
                 <tr>
                   ...
                   <td>Total Horizontal (in): {total_horizontal:.2f} × Cost</td>
                 ...
                 <tr>
                   ...
                   <td>
                     ...
                     × Total Horizontal (in): {total_horizontal:.2f} × Cost
                   </td>
                   <td>{cost_tape:.2f}</td>
                 </tr>
                 <tr>
                   <th colspan="4">Material Total Cost</th>
      ...
      <body>
         <div class="container">
            ...
            <form method="POST">
               ...
               <!-- Aluminum: option appears above the picker -->
               ...
               <label for="material_gaskets">Gaskets (Cat 3 - Total Vertical (in) × Cost):</label>
               ...
               <label for="material_foam_baffle">Foam Baffle (Cat 6 - Total Horizontal (in) × Cost):</label>
               <select name="material_foam_baffle" id="material_foam_baffle" required>
                  {options_foam_baffle}
               </select>

               <label for="material_glass_protection">Glass Protection (Cat 7 - Total Horizontal (in) × Cost):</label>
               ...
               <button type="submit">Calculate Material Costs</button>
            </form>
...
def other_costs():
    ...
    if request.method == 'POST':
        ...
        # Store additional costs in current_project
         ...
         <html>
           ...
           <body>
             <div class="container">
               ...
               <button onclick="window.location.href='/materials'">Back to Materials</button>
             </div>
           </body>
         </html>
        """
      ...
      <body>
         <div class="container">
            ...
            <form method="POST">
              ...
              <input type="number" step="1" id="num_trucks" name="num_trucks" required>
              <label for="truck_cost">Cost per Truck ($):</label>
              <input type="number" step="0.01" id="truck_cost" name="truck_cost" required>
              ...
              <input type="number" step="0.1" id="hours_per_panel" name="hours_per_panel" required>
              ...
              <input type="number" step="0.01" id="airfare" name="airfare" required>
              <label for="lodging">Lodging ($):</label>
              <input type="number" step="0.01" id="lodging" name="lodging" required>
              <label for="meals">Meals &amp; Incidentals ($):</label>
              <input type="number" step="0.01" id="meals" name="meals" required>
              <label for="car_rental">Car Rental + Gas ($):</label>
              <input type="number" step="0.01" id="car_rental" name="car_rental" required>

              <h3>Sales</h3>
              <p>Enter cost for each sales item (0 if not needed):</p>
              <label for="cost_audit">Building Audit/Survey ($):</label>
              <input type="number" step="0.01" id="cost_audit" name="cost_audit" value="0">
              <label for="cost_design">System Design Customization ($):</label>
              <input type="number" step="0.01" id="cost_design" name="cost_design" value="0">
              <label for="cost_thermal_stress">Thermal Stress Analysis ($):</label>
              <input type="number" step="0.01" id="cost_thermal_stress" name="cost_thermal_stress" value="0">
              <label for="cost_thermal_performance">Thermal Performance Simulation ($):</label>
              <input type="number" step="0.01" id="cost_thermal_performance" name="cost_thermal_performance" value="0">
              <label for="cost_mockup">Visual &amp; Performance Mockup ($):</label>
              <input type="number" step="0.01" id="cost_mockup" name="cost_mockup" value="0">
              <label for="cost_window">Window Performance M&amp;V ($):</label>
              <input type="number" step="0.01" id="cost_window" name="cost_window" value="0">
              <label for="cost_energy">Building Energy Model ($):</label>
              <input type="number" step="0.01" id="cost_energy" name="cost_energy" value="0">
              <label for="cost_cost_benefit">Cost-Benefit Analysis ($):</label>
              <input type="number" step="0.01" id="cost_cost_benefit" name="cost_cost_benefit" value="0">
              <label for="cost_utility">Utility Incentive Application ($):</label>
              <input type="number" step="0.01" id="cost_utility" name="cost_utility" value="0">
...
def margins():
    # Base cost values from current_project
    base_costs = {
    ...
    if request.method == 'POST':
        # Get margin percentages from the form (as floats)
        margins = {}
        ...
        for category, cost in base_costs.items():
            adjusted_costs[category] = cost * (1 + margins[category] / 100)
        final_total = sum(adjusted_costs.values())

        # Create CSV summary without using deprecated .append
        summary_data = {
    ...
    # GET: Display margin sliders using range inputs with real-time output
    form_html = f"""
    <html>
      <head>
